...
server <- function(input, output) {
  output$mapPlot <- renderPlot({
    ...
    map <- base_map +
      geom_point(data = data_for_plot, aes(x = longitude, y = latitude, size = size), alpha = 0.5) +
...
```{r}
ui <- fluidPage(
    selectInput("y", "Select Y Variable", choices = c("total", "fatality", "injury", "pop", "total_capita", "fatality_capita", "injury_capita"), 
                selected = "total"),
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ...
    data_ordered <- bikes_capita %>%
      mutate(county_reordered = reorder(county, get(input$y), FUN = function(x)-mean(x))) # Use -mean(x) for descending order
